...
/**
 * This class gets rid of file length checking from SequenceFile.Reader, because when syncFs is used, the file
 * length is not updated on the namenode. This makes SequenceFile.Reader incapable of reading records
 * written by SequenceFile.Writer that *were synced*.
 * 
 * This class works around these insane semantics and will just throw EOFException when it reaches the end of
 * the file.
 */
public class SequenceFileReader implements java.io.Closeable {
...
private static class CompressedBytes implements ValueBytes {
  ...
  private static final Log LOG = LogFactory.getLog(SequenceFileReader.class);
  ...
  public static final int SYNC_INTERVAL = 100*SYNC_SIZE; 
    ...
    /** Open the named file. */
    public SequenceFileReader(FileSystem fs, Path file, Configuration conf)
            throws IOException {
        this(fs, file, conf.getInt("io.file.buffer.size", 4096), conf, false);
    }

    private SequenceFileReader(FileSystem fs, Path file, int bufferSize,
            Configuration conf, boolean tempReader) throws IOException {
        this(fs, file, bufferSize, 0, fs.getLength(file), conf, tempReader);
    }

    private SequenceFileReader(FileSystem fs, Path file, int bufferSize, long start,
